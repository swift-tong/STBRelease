...
class CheckUserInput():
    ...
    def getProductPropertyDict(self):
        productDict={key[0]:key[1] for key in tuple(intmapext.IntToExtDict.keys())}
        for key in productDict.keys():
            valueList=[]
            if key == "S-010W-A":
                valueList.append(productDict[key])
                valueList.append(('RK3228H','8189FTV'))
                productDict[key]=valueList
            else:
                valueList.append(productDict[key])
                productDict[key]=valueList
        return productDict
    ...
    def checkInput(self):
        ...
        productPropertyDict=self.getProductPropertyDict()
        ...
        if "_" in self.swVersion:
            a_tup=productPropertyDict[self.swVersion.split("_SW")[0]]
            self.logger.info("test={}".format(str(a_tup)))
            self.logger.info("a_tup[0]={}".format(a_tup[0]))
            #if ((self.chipset,self.wifiType)) not in a_tup and (('S905L2', 'RTL8822BS') not in a_tup) and (('S905L3', 'RTL8822BS') not in a_tup):
            if a_tup[0] not in [(self.chipset,self.wifiType),('S905L2', 'RTL8822BS'),('S905L3', 'RTL8822BS'),("RK3228B","RTL8189ETV"),('S905L','RTL8676'),('S905L3', 'RTL8676')]:
